...
{
    error::Error, future::poll_fn, mem::swap, process::Output, result, sync::atomic::Ordering, task::{Poll, Waker}
};
...
impl<'t, F> ReporterGuard<'t, 'static, F, AsyncParts>
where
    F: ThreadModel
{
...
impl<'t, M: ThreadModel> ReporterBusy<'t, 'static, M, AsyncParts>
where
    M: ThreadModel,
{
...
impl<'t, M: ThreadModel> IdleReporter<'t, 'static, M, AsyncParts>
where
    M: ThreadModel
{
...
async fn retry_loop<S: DownloadStream>(reporter: &Reporter<'static, F, AsyncParts>, mut stream: Option<S>) -> Result<(),()> {
    loop {
...
async fn handle_stream_optional<S: DownloadStream, F: ThreadModel>(reporter: &Reporter<'static, F, AsyncParts>, stream: Option<S>) -> Result<(), ()> {
...
///创建连接
async fn fetch_normal_stream<F: ThreadModel>(reporter: &Reporter<'static, F, AsyncParts>) -> impl DownloadStream {
    todo!()
}

///写入下载流
async fn handle_stream<S: DownloadStream, F: ThreadModel>(reporter: &Reporter<'static, F, AsyncParts>, stream: S) -> Result<(), ()> {
...
// type SubResult<T> = Result<Result<T, SupError>, Retry>

// type SuperResult2<T> = Result<Result<T, SuperError>, Aborted>
...
// ///在出错结束时保存下载进度
// trait Save{
//     fn save(&mut self, segments: impl Iterator<Item = Segment>);
// }
